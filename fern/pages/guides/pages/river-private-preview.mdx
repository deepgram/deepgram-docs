---
title: "Private Preview: Realtime Conversational STT for Voice Agents"
...
## Codename River: Realtime STT Designed for Human-Like Conversation
...
River is designed for teams building real-time voice agents requiring turn-based natural conversation flow, interruption handling, and ultra-low latency human-voice agent interaction. It is *not designed* for multi-speaker audio like meeting transcription, pure transcription products like closed captioning where real-time conversational turn-taking dynamics do not apply, or products requiring batch transcription or long tail feature support.
...
River enables two voice agent patterns - choose based on your latency vs complexity/cost tradeoffs.

**Simple Approach: `EndOfTurn` Only (higher latency, simpler logic, fewer LLM calls)**

We recommend starting with a purely `EndOfTurn` driven implementation to get up and running. This means:

- Ignore or only use 'Update', `Preflight`, and `SpeechResumed` messages for referencing the current transcript
...
**Optimized Approach: Preflight + EndOfTurn (lower latency, slightly more complex, more LLM calls)**
...
### Why River's State Machine Matters

Traditional STT+VAD requires you to build complex interruption logic. River handles this natively. Emitted events adhere to the below state machine for managing turns:
...
3. A `SpeechResumed` message can only follow a preceding `Preflight` message
