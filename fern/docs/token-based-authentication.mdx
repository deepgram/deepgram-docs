...
<Info>
If you need a short-lived ephemeral token, we highly recommend using [token-based authentication](/reference/ephemeral-auth-api/grant-token) over temporary API keys.
</Info>
...
- Secure sharing of API access with clients when combined with a backend proxy
...
Ephemeral tokens are preferred over temporary API keys because they are ephemeral: they are expired and removed after you are done using them.
