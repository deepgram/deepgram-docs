...
| `agent.think.provider.type` | Object | The [LLM Model](/docs/voice-agent-llm-models) provider type e.g., `open_ai`, `anthropic`, `x_ai`|
    ...
    "agent": {
      ...
      "speak": {
        "provider": {
          ...
          "engine": "standard", // For AWS Polly provider only
          "credentials": { // For AWS Polly provider only
            "type": "IAM", // For AWS Polly provider only. Must be "IAM" or "STS"
            "region": "us-east-1",
