...
* If an `endpoint` is provided the `url` and `headers` are required.
...
<CodeGroup>
  ...
  "think": {
        ...
        "endpoint": { // Required for custom LLM
          "url": "https://cloud.provider.com/llm", // Required for custom LLM
          "headers": { // Required for custom LLM
            "authorization": "Bearer {{token}}"
          }
        },
    // ... other settings ...
