...
In the steps below, we'll use the Terminal to:

1. Create a new directory for our project
2. Create a new file for our code
3. Export our Deepgram API key to the environment so we can use it in our code
4. Run any additional commands to initialize our project
...
touch index.cs
export DEEPGRAM_API_KEY="your_Deepgram_API_key_here"
@TODO other C# commands

# for Windows Powershell
mkdir deepgram-agent-demo
cd deepgram-agent-demo
New-Item index.cs -ItemType File
...
Next, we'll import the necessary dependencies and set up our main application function.
...
Now we'll initialize the voice agent by creating an empty audio buffer to store incoming audio data, setting up a counter for output file naming, and defining a sample audio file URL. We can then establish a connection to Deepgram and set up a welcome handler to log when the connection is successfully established.

<Info>
This example code doesn't use microphone audio input as that is much more complex and requires additional knowledge of how to handle audio input through a microphone.
</Info>
...
Next we need to set up a very simplified version of the [Settings](/docs/voice-agent-settings) message to configure our Agent's behavior and set the required settings options for our Agent.
...
Next we'll send a keep-alive signal every 5 seconds to maintain the WebSocket connection. This prevents the connection from timing out during long audio processing. We will also fetch an audio file from the specified URL [spacewalk.wav](https://dpgr.am/spacewalk.wav) and stream the audio data in chunks to the Agent. Each chunk is sent as it becomes available in the readable stream.
...
Next we'll use this code to set up event handlers for the voice agent to manage the entire conversation lifecycle, from connection opening to closing. It handles audio processing by collecting chunks into a buffer and saving completed responses as WAV files, while also managing interruptions, logging conversations, and handling errors.
...
Now that we have our complete code, we can run the Voice Agent! If it works you should see the conversation text and audio in the files: `output-0.wav` and `chatlog.txt`. These files will be saved in the same directory as your main application file.
...
Below is the final code for the Voice Agent we just built. If you saw any errors after running your Agent, you can compare the code below to the code you wrote in the steps above to find and fix the errors.
