...
program
  ...
  .option('-p, --path <filepath>', 'Process a specific page by its path in docs.yml (e.g., docs/diarization.mdx)')
...
function extractSlugFromFile(filePath) {
  ...
  try {
    ...
    if (data.slug) {
      log(`Removing old slug from file (we need to remember what we remove for later): ${data.slug}`);
      return data.slug;
    } else {
...
function generateTargetPath(fullPath, filePath) {
  ...
  const httpComponents = fullPath.substring(1).split('/');
    ...
    if (options.path) {
      ...
      if (!options.dryRun) {
        const { confirm } = await inquirer.prompt([{
          type: 'confirm',
          name: 'confirm',
          message: 'Are you sure you want to process ALL pages? This will modify files and update docs.yml.',
          default: false
        }]);
